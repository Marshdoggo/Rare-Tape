...
- Rally Leaderboards Lab provides a reproducible, long-format quarterly state archive with current rankings, a rank-over-time chart, percentile ranks, eligibility gaps, movement/entry/dropout analysis, exclusions, and CSV downloads. Its offline builder truncates every source at actual dates on or before each calendar quarter-end, applies a transparent 186-day staleness rule, and stores raw metric values, deterministic ordinal ranks, state diagnostics, source fingerprints, and methodology versions. The generated Parquet archive is an ignored local cache rather than a committed deployment dependency; when absent, Streamlit reconstructs it once from committed canonical inputs and memory-caches the result. It supports canonical individual assets, existing quarterly equal-weight/category, market-cap-weight/category, and full-market index prototypes, plus locally persisted Benchmark Lab subjects. Historical status is explicitly inferred rather than claimed as contemporaneous, and historical market cap is labeled as an estimate from available share-count metadata. See `docs/LEADERBOARDS_METHODOLOGY.md`.
