...
The application brand is **Rare Tape** (slug `rare-tape`; code identifier `rare_tape`). The existing `alt_asset_explorer` Python import path and `app/Home.py` deployment entrypoint remain in place for backward compatibility. References to Rally, Rally Rd., Rally asset IDs, and Rally source fields identify the underlying exchange or data provenance—not the Rare Tape product brand.
...
Derivatives Lab values are experimental model estimates, not observed option-market premiums, appraisals, forecasts, or executable prices. Canonical data supplies authored share-price observations and, where available, static share counts and lifecycle state; it does not supply a live option chain, option volume/open interest, firm bid/ask quotes, borrow availability, contractual distribution rules, or executable hedges. Synthetic index levels are not transferable instruments. Sparse and stale collectible observations, jumps, irregular trading windows, transfer restrictions, paused/exited assets, and unknown settlement mechanics materially limit model interpretation.

A future paper exchange is deliberately outside the shipped scope. Any movement toward a real market would first require Rally approval, broker-dealer and legal review, custody, share locking, collateral rules, confirmed transferability, settlement and assignment mechanics, exercise notices, default procedures, market surveillance, governing rules, customer eligibility controls, and regulatory classification. User accounts, order books, matching, bots, persistent simulated trades, payments, and counterparties are not part of Derivatives Lab 2.0.
...
Content Lab is a deterministic analytical story-discovery layer, not an LLM or copywriting surface. Build 1.5 adds conservative correlation and volatility regimes, streak/rank history, temporally dated experimental fair-value gaps, canonical index attribution/concentration, weighting-method divergence, and confirmed-exit SPY holding-period comparison to the initial detector slate. Evidence packets now expose comparison/historical context, explicit temporal validity, primary/secondary angles, detector-specific research questions, and visualization recommendations. Configurable bounded scoring includes advanced rarity/regime/rank/gap/concentration/excess dimensions and a multiplicative evidence-quality penalty, followed by stable subject/family deduplication and slate diversification.

Historical operation is contemporaneous by default: actual observation, benchmark, exit, rank, and valuation availability dates are truncated at the selected boundary, and stale marks beyond 186 days are excluded. Valuations require an authored valuation date or low-confidence manifest availability timestamp; filesystem mtime is never authoritative. Advanced families fail closed when overlap or temporal validity is insufficient. Generated CSV/JSON archives under `data/processed/content_lab/` are committed runtime inputs and can be rebuilt for all, latest, or selected quarters. See `docs/CONTENT_LAB.md` for the full contract and limitations.

The canonical dataset rebuild now regenerates the complete Content Lab archive after processed indexes and quarterly leaderboards. Manual-research ingestion and future asset-data uploads that use this shared rebuild graph therefore flow into story discovery automatically from normalized repository inputs, without category or ticker allowlists.

Build 2 adds an optional, evidence-packet-only OpenAI Responses API Intelligence Layer behind typed domain and cache adapters. It produces structured research/content briefs with claim audits, provenance, token usage, family-aware prompts, numeric review warnings, and explicit fact/interpretation/hypothesis/unknown boundaries. Streamlit is cache-first and never generates on load; deterministic Content Lab remains fully usable without credentials or network access. Local reports are ignored runtime JSON under `data/processed/content_lab/intelligence/`; this filesystem is ephemeral on Streamlit Community Cloud, so durable multi-instance persistence requires a future backend. See `docs/INTELLIGENCE_LAYER.md`.
...
## Trading Cards Asset Registry Coverage (2026-08-14)
...
An additional 100 supplied observations cover five more active Comics securities: `BATMAN2` (21), `SPIDER1` (25), `BATMAN181` (18), `PENGUIN` (18), and `SURFER4` (18). The closest-to-quarter-end method selects 98 quarterly representatives; BATMAN2's October 27, 2021 and SPIDER1's January 24, 2022 observations remain dated non-quarterly evidence because later observations in those quarters are closer to quarter end. BATMAN2's specifically supplied May 10, 2021 row retains offering-event semantics because its price and implied market cap reconcile to the registry offering, without inventing another offering-day row. SPIDER1 begins with the supplied $27 secondary-market observation rather than its separate $22 registry offering price. Corrected PENGUIN observations and SPIDER1's confirmed 2023 volatility are retained as supplied. All five tickers match exactly one existing registry record, all implied market caps reconcile to authoritative shares, and registry metadata remains unchanged.

Comics price-history coverage is now ten of 38 registry assets and ten of 23 active Comics; the two pending-buyout and 13 exited records remain unchanged. Across the ten covered assets, 216 dated observations yield 210 quarterly representatives and six retained same-quarter observations. Comics equal-weight and market-cap-weighted indexes, full-market indexes, exchange market-cap history, research coverage, leaderboards, correlation, scatter, benchmark, portfolio, integer-replication, contribution, AI context, and export consumers were rebuilt or refreshed through their canonical inputs. Pending offer values remain non-trade registry metadata and do not populate exit fields, realized returns, or price history.

The third Comics history batch adds 157 supplied observations for eight more active registry securities: `SPIDER129` (19), `SHOWCASE4` (17), `HULK180` (18), `59FLASH` (21), `AC23` (20), `SPIDER10` (22), `FLASH123` (21), and `XLXMEN1` (19). Every ticker matched exactly one existing registry record, with `59FLASH` retained as canonical and `HULK180` kept distinct from halted `WOLVERINE`; registry metadata was not changed. All dates and prices are preserved exactly, and implied market caps reconcile to registry shares. The nearest-quarter methodology selects 156 representatives: FLASH123's January 2, 2025 observation represents Q4 2024 because it is closer to quarter end, while its December 26 observation remains non-quarterly evidence; SPIDER10's July 1, 2026 observation represents Q2 2026. No offering-day observations were invented, including for SHOWCASE4. Comics coverage is now 18 of 23 active securities (18 of 38 historical registry records), with 373 raw observations and 366 quarterly representatives. The two pending-buyout and 13 exited records remain unchanged. Canonical Comics and full-market indexes plus runtime consumers were refreshed through the deterministic rebuild graph.

The final Comics history batch adds all 116 supplied observations for `XMEN94` (20), `DRACULA10` (16), `GHOST1` (19), `BATMAN6` (20), `CAPTAIN3` (24), and `SUPERMAN6` (17), completing authored price coverage for all 24 currently listed Comics securities (23 trading and halted/pending `BATMAN6`). The nearest-quarter methodology selects 114 representatives: CAPTAIN3's November 17, 2020 row and SUPERMAN6's November 21, 2022 row remain same-quarter evidence, while CAPTAIN3's July 1, 2026 observation represents Q2 2026. BATMAN6 intentionally has no Q2 2025 row; its false December 9, 2021 record was not ingested, and its pending $11 offer remains registry metadata rather than a trade or realized exit. Comics now has 489 raw observations and 480 quarterly representatives across currently listed securities, without interpolation or forward-filling. Registry identity and lifecycle metadata remain unchanged, implied market caps use canonical shares, and canonical category/full-market indexes plus runtime consumers are refreshed through the deterministic rebuild graph.

## Handbags Pending Buyout Coverage (2026-08-09)

The canonical Handbags Asset Registry records pending buyout offers for `BIRKINBOR`, `HIMALAYA`, and `PICNIC`. All three use their existing 2,000-share masters and are `buyout_pending` / `halted`, with pending `buyout_offer` lifecycle events. Their respective $49,000, $120,000, and $31,000 total offers imply $24.50, $60.00, and $15.50 per share. Reference prices remain the latest accepted market observations: $17.00 on June 26, 2026; $46.90 on June 26, 2026; and $13.30 on June 29, 2026. Offer economics are lifecycle metadata only: no offer was appended to price history, recognized as a completed exit, used to create realized returns, or substituted into historical Handbags indexes. Historical-universe analysis retains these securities while active-trading filters exclude them.
...
The normalized Rally observation layer now includes 131 manually transcribed dated observations for five additional existing Cars masters: `65AG1` (26), `82AV1` (18), `69BM1` (29), `92LD1` (29), and `92CC1` (29). Their authoritative master metadata was not changed. All price-derived market caps reconcile to master shares outstanding, and offering observations that match master offering terms retain offering-event semantics. The first `69BM1` row is an offering reference at $57.50 per share and $115,000 market cap; an earlier transcription incorrectly treated the market-cap figure as a $115 share price and has been corrected. Exact observation dates and sparse histories are preserved without interpolation or forward-filling. The closest-to-quarter-end methodology selects 127 quarterly representatives while retaining four additional same-quarter observations as weekly evidence, including both `92CC1` observations around the 2024 Q1 and Q2 boundaries. Cars now has 20 master assets, ten of which have authored price history. Equal-weighted and market-cap-weighted Cars indexes, coverage, leaderboards, market-cap and return analytics, correlation inputs, portfolio inputs, and dependent processed artifacts were rebuilt.

## Manual Cars Price Coverage Completion (2026-07-31)

The normalized Rally observation layer now includes all 281 supplied dated observations for the ten remaining Cars masters: `61JE1` (28), `61MG1` (26), `72MC1` (28), `75RA1` (26), `77LE1` (34), `11BM1` (28), `94FS1` (26), `99SS1` (27), `91MV1` (29), and `94DV1` (29). The Jaguar E-Type master identifier is corrected in place to canonical ticker `61JE1`; no duplicate master was created. The established closest-to-quarter-end methodology selects 273 quarterly representatives and retains eight additional same-quarter observations as dated weekly evidence. Sparse periods, including `11BM1` Q1 2022, remain absent without interpolation or forward-filling. All implied market caps reconcile to authoritative master shares. Cars research coverage is now 20 of 20 currently trading masters, and Cars equal-weight, market-cap-weighted, full-market, coverage, leaderboard, correlation, portfolio, contribution, scatter, benchmark-context, AI-context, and export consumers were rebuilt or refreshed through their canonical inputs. No buyout offer, vote, exit, or lifecycle metadata was added or changed.


### Asset Valuation Library Auto-Enrichment (2026-08-05)

The valuation-library intake now uses one canonical identity resolver across validation, save, manifest refresh, and valuation execution. Uploaded aliases such as ticker-case variants are normalized to the selected canonical asset ID while retaining input provenance, so the UI selection remains authoritative and stale intake state is cleared when the selected asset changes. Intake persistence is transaction-like: it validates and stages factors/research before replacing final files, blocks all writes when overwrite is disabled and target files exist, creates timestamped revision backups when overwrite is enabled, and records recoverable `valuation_error` manifest state if the valuation engine fails after intake files are saved. Mixed-type Streamlit display tables are formatted through a display-only normalization boundary so structured storage remains typed while PyArrow receives string-safe dataframe values.

The valuation-library intake now resolves a selected Rally asset against the committed canonical asset master and processed price history before accepting manual specifications. `get_asset_financial_context()` reads `data/processed/canonical_asset_master.csv` for identity, category, lifecycle/status, offering date, share count, initial offering price, and initial offering valuation, and reads `data/processed/price_history.csv` for latest share price, latest market value, last observed trade date, and quarterly price observations keyed by canonical `asset_id`. The asset-factor assembler preserves these repository-derived fields under `rally_data`, stores user-transcribed collectible specifications under `category_factors`, records source provenance at the section level, and retains canonical values when full pasted factors conflict with repository data. Streamlit displays the existing Rare Tape data in a read-only intake panel and labels the JSON text area as supplemental asset specifications so users are not expected to re-enter financial or quarterly price data already present in Rare Tape. Detailed inspection fields such as polishing history, dial surface condition, bracelet stretch, service/restoration details, independent expert inspection, original box, and full ownership provenance remain optional: their absence generates warnings and may lower confidence or produce `completed_with_limitations`, but does not by itself make evidence insufficient when defensible completed comparables exist. `insufficient_evidence` remains reserved for cases such as no eligible completed comparables or unresolved/contradictory core evidence. ChatGPT report packages include the enriched full `factors.json` alongside research, valuation, and drafting instructions.

The valuation engine now emits per-comparable eligibility diagnostics before final filtering, including parsed sale status, USD price, similarity, evidence quality, verification multiplier, recency treatment, currency conversion source, exclusion reasons, and final weight. The Valuation Library detail page renders these diagnostics as a table for methodology debugging. Category methodology YAML can define similarity aliases; the handbags model maps handbag-specific fields such as model, size, material, hardware, year, accessory, condition, and overall similarity into generic identity, condition, provenance, and presentation components without silently discarding supplied category evidence. Supported non-USD completed sales can use supplied `price_usd` / `price_usd_at_sale`, research-supplied `fx_rate_to_usd`, or the deterministic engine FX table for USD, HKD, CHF, GBP, and EUR; missing or unsupported conversion remains an explicit exclusion reason rather than a silent drop. Unverified sales are generally down-weighted by the configured multiplier rather than automatically excluded, while analyst `eligible_for_official_valuation=false` remains an exclusion gate after core sale, price, similarity, and evidence checks.

Revised valuation workflow: select the Rally asset; review automatically loaded Rare Tape data; paste only supplemental asset specifications from Rally Rd; paste `research.json`; validate and save; run valuation; download the ChatGPT Report Package; generate `report.md` in ChatGPT; return and ingest `report.md`. Financial and historical fields remain sourced from committed Rare Tape artifacts rather than a second valuation-specific database.

### Asset Valuation Library (2026-08-04)

Rare Tape now includes an asset-level valuation-library foundation under `data/valuation_library/<ASSET_ID>/`. The feature intentionally separates structured asset factors (`factors.json`), research evidence (`research.json`), deterministic valuation output (`valuation.json`), and narrative reports (`report.md`). Streamlit exposes the library through `app/pages/8_Valuation_Library.py`, while reusable Python modules in `src/alt_asset_explorer/valuation_library/` provide validation, asset resolution against the canonical Rally registry, revision-safe storage, manifest regeneration, ChatGPT report-package downloads, and the deterministic v1 weighted-comparable engine. Methodology assumptions are versioned in `methodology/` and remain provisional; insufficient comparable evidence produces an `insufficient_evidence` status rather than a definitive fair-value appraisal. Runtime reads remain limited to committed processed/normalized/report artifacts plus committed valuation-library records; no OpenAI API, automated web research, or live data fetch is added.
...
- Rally Derivatives Lab 2.0 dynamically discovers sufficiently supported canonical authored assets and committed equal-/market-cap-weighted category and full-market quarterly index prototypes. It defaults to canonical `rally-marx` / `MARX` when eligible. The modular, research-only workflow supports European calls and puts through Black-Scholes-Merton and CRR, American calls and puts through CRR node-level early-exercise checks, 10- and 100-share hypothetical multipliers, multiple contracts, long-option/covered-call/protective-put views, analytic European and numerical American Greeks, and Actual/365 controls. Index claims are synthetic/non-tradable and all outputs retain legal, settlement, liquidity, stale-price, and model-risk caveats.

  The deployed `app/pages/6_Derivatives_Lab.py` surface exposes that catalog through a searchable, canonical-ID selector above the summary cards. Human-readable labels identify the name, ticker or index ID, category, and asset/index type. Changing underlyings clears only price-, history-, volatility-, strike-, premium-, and expiration-dependent widget state while preserving compatible contract choices. An availability diagnostic reports eligible asset/index counts and catalog exclusions instead of silently degrading to MARX. Synthetic indexes remain cash-settled research assumptions with no shares or covered-contract capacity.

  The volatility workbench preserves authored observations and actual inferred timing without interpolation or forward filling. Historical volatility is the sample dispersion of consecutive returns annualized from median spacing; unstable samples are warned or suppressed. Manual assumptions remain available. Implied volatility is solved by bounded bisection only from a user-entered hypothetical premium—never inferred from underlying history alone—and validates model bounds. Risk-neutral model-implied ITM probability is kept distinct from a reproducible historical-return bootstrap probability of profit; the latter preserves inferred frequency, uses 5,000 paths and seed 42 in the UI, and is suppressed below eight returns.

  Transparent, reusable 0–100 liquidity and model-confidence diagnostics expose component values and deductions; they are not credit ratings, suitability decisions, or regulatory classifications. The hypothetical spread heuristic widens theoretical value for weak liquidity/confidence, term, moneyness, volatility, multiplier, staleness, and hedge infeasibility, floors bid at zero/basic intrinsic value, and is never described as a quote. Capacity is only `floor(shares_outstanding / multiplier)` for individual assets with canonical positive share counts; it is unavailable for indexes or missing counts and does not establish transferability, borrow, availability, or collateral eligibility. Payoff charts cover all four strategies at selected position scale, with scenario evidence and a bounded underlying-price/time value surface. The in-page term sheet remains explicitly hypothetical, non-executable, non-signable, and disconnected from payments and counterparties.
...
- Portfolio analytics now live in a dedicated tested module. The typed backtest result exposes drawdown, turnover, rebalance, cash, and arithmetic contribution histories alongside eligibility, look-through, overlap, and weight-reconciliation evidence; the homepage renders the complete contract. Risk contribution and advanced attribution remain gated until named deterministic reconciliation fixtures are committed and pass exactly.
...
- Saved portfolios and constituent strategies now have a separate, strict version-1 document contract and JSON storage adapter; they do not extend or share the existing custom-index files. The reproducibility contract records component and exclusion rules, weighting, alignment, eligibility, exit and rebalance policies, the as-of information cutoff, dataset version, and methodology version. Stored optimization evidence is accepted only as explicitly labeled in-sample or walk-forward historical research, never as an unlabeled forecast.
- Benchmark Lab provides a reusable, tested analytics boundary for comparing canonical individual-asset observations and exit-aware full-market/category total-return series with public-market proxies. A committed long-format benchmark history in `data/processed/benchmark_history.parquet` is the production read path, with CSV and explicitly enabled live Yahoo retrieval as fallbacks. `scripts/update_benchmark_data.py` derives its fetch start from valid authored Rally observations and supports incremental and full refreshes; a weekly/manual GitHub workflow refreshes and validates the artifact. Benchmark observations can be matched exactly, sampled using the previous close known at each Rally evidence date, or reduced to month/quarter ends. Rally observations are never forward-filled. The page exposes freshness provenance, growth, relative wealth, rolling excess, period returns, drawdowns, frequency-aware absolute and relative statistics, aligned evidence, and a reproducible CSV export.
- Modular Scatter Plot Explorer provides a reusable asset-level cross-sectional metric table and a configurable Plotly research view over canonical assets and authored observations. Its default risk-return map places annualized volatility on X and annualized arithmetic mean return on Y, uses uniform 10-pixel category-colored markers, maximum available individual history, active assets, six valid returns, and filtered-sample median guides. Optional numeric sizing is display-only log-transformed, 95th-percentile clipped, and bounded to 8–30 pixel diameters while raw values remain in hover/export. Regular histories use conventional frequency inferred from median spacing; irregular histories use observed return intervals per elapsed year, consistently for arithmetic return and volatility. The module does not interpolate prices or claim live listings. Index subjects remain deliberately gated until the index metadata contract can support the same cross-sectional interface without duplicating canonical index logic.
- Correlation Structure Lab provides a reusable alignment, return, pairwise-correlation, complete-subset, correlation-distance, hierarchical-clustering, and export boundary. It combines canonical authored individual-asset prices, the committed equal- and market-cap-weighted quarterly category/full-market index prototypes, canonical quarterly exit-aware full-market variants, and the persisted Benchmark Lab history without request-time network access. Quarterly previous-available alignment, simple returns, Pearson correlation, six overlapping returns, square-root correlation distance, average linkage, and a 180-day maximum staleness rule are defaults. Missing or constant correlations remain unavailable; the dendrogram deterministically removes the least-connected subjects until its matrix is complete while the heatmap retains partial cells. The Streamlit page includes an interactive Plotly dendrogram and same-matrix heatmap, pair discovery, diagnostics, explicit quality limiting, filters, and five CSV exports. These correlations are descriptive research over sparse/event-driven values, not forecasts or appraisals.
- Rally Leaderboards Lab provides a reproducible, long-format quarterly state archive with current rankings, a rank-over-time chart, percentile ranks, eligibility gaps, movement/entry/dropout analysis, exclusions, and CSV downloads. Its offline builder truncates every source at actual dates on or before each calendar quarter-end, applies a transparent 186-day staleness rule, and stores raw metric values, deterministic ordinal ranks, state diagnostics, source fingerprints, and methodology versions. The generated Parquet archive is an ignored local cache rather than a committed deployment dependency. Streamlit validates that archive and keys its in-memory cache with a fingerprint of canonical assets, observations, quarterly indices, and committed benchmark history; changed inputs now force reconstruction rather than serving an older subject universe. It supports canonical individual assets, existing quarterly equal-weight/category, market-cap-weight/category, and full-market index prototypes, plus locally persisted Benchmark Lab subjects. Historical status is explicitly inferred rather than claimed as contemporaneous, and historical market cap is labeled as an estimate where share-count history is unavailable. See `docs/LEADERBOARDS_METHODOLOGY.md`.
- Individual Rally Asset leaderboard membership is dynamically discovered from the canonical normalized asset and observation tables, without a category allowlist or static subject registry. The 2026-06-30 archive was refreshed after Coins ingestion; `1857COIN`, `JUSTINIAN`, and `CROESUS` now flow through as eligible `coins` subjects, and `1857COIN` reconciles to the Market Table methodology at a 220% inception price return from $5 to $16.
- The five Comics histories (`SUPER21`, `BATMAN3`, `STARWARS1`, `AVENGERS57`, and `FAN45`) are integration-guarded as eligible individual subjects at the latest completed quarter. A prior zero-argument Streamlit cache and unvalidated ignored archive could survive canonical-data changes; both cache layers are now invalidated by the complete leaderboard source fingerprint.
- The standard `python scripts/build_dataset.py` command and successful manual-research ingestion now share one deterministic orchestrator. It rebuilds processed research/index/export/AI artifacts first, research coverage second, and the quarterly leaderboard archive last. Correlation, scatter, portfolio, custom-index, benchmark-comparison, contribution, market-table, sector, and exchange views derive their Rally side from canonical normalized inputs or those freshly regenerated outputs rather than page-owned history snapshots. The Correlation Structure Lab retains its observation-count quality ranking while allowing up to 300 subjects.
...
- Benchmark history refreshes still depend on Yahoo Finance's unauthenticated chart endpoint, which offers no service-level guarantee, but ordinary deployed Benchmark Lab reads use the committed artifact and do not require a request-time provider call. Persisted strategy documents still need a stable, methodology-owning history resolver before they can safely appear as comparison subjects.
...
## Exited Books Master Coverage (2026-07-27)

The normalized Books master now contains 45 unique historical assets: the existing 40 currently trading records and five exited records (`CATCHER`, `EINSTEIN`, `FEDERAL`, `HOBBIT`, and `POTTER`). Their offering share counts, prices, valuations, exit dates, exact exit values, derived per-share proceeds, and decimal realized returns are preserved in canonical source or processed exit fields. POTTER's $132,500 exact exit value is authoritative and produces the repeating per-share value; $44.17 is display rounding only. No quarterly observations were added for these five records. Active-only universes therefore retain the 40 trading Books, while include-exited historical master views can resolve all 45; price-history-dependent simulations will continue to identify the five exited records as missing history until authored observations are ingested.

## Portfolio Phase 0 Production Diagnostic (2026-07-26)

The Portfolio Laboratory now has a deterministic Phase 0 diagnostic over the committed normalized schemas, before any backtest or derived-index transformation. The verified currently trading Books snapshot contains exactly 40 normalized asset IDs and 850 authored observation rows. All 40 IDs resolve; a deliberately unknown test ID remains explicitly missing. The five exited Books master additions are intentionally outside this price-history diagnostic until observations are authored. There are no common actual `observed_at` dates across all 40 assets. After selecting quarterly rows and resolving four asset/period collisions by retaining the latest actual observation in each canonical period, 837 canonical rows remain from 841 quarterly rows and the all-asset canonical-period intersection contains 12 periods: 2023 Q1 through 2024 Q4, 2025 Q1, 2025 Q4, 2026 Q1, and 2026 Q2. There are no duplicate asset/actual-observation-date rows in this snapshot. Canonical launch periods range from 2019 Q4 through 2022 Q3, while every authored Books series currently ends at the 2026 Q2 canonical period.
...
## Asset Valuation Library Comparable Ingestion Diagnostics (2026-08-05)

### Comparable FX Field Mapping Update (2026-08-05)

Comparable-sale ingestion now accepts canonical research FX fields (`reported_currency`, `reported_realized_price`, `price_usd_at_sale`, `fx_rate_to_usd`, and `fx_conversion_required`) while retaining legacy aliases (`currency`, `reported_price`, `original_reported_price`, and `price_usd`). The valuation engine preserves non-USD reported currencies through diagnostics and normalizes comparable prices deterministically by using a supplied USD sale price first, then a supplied dated FX rate, then the versioned configured FX table for USD, HKD, CHF, GBP, and EUR. Records that still cannot be USD-normalized are excluded with an explicit `missing_usd_normalized_price` reason rather than being mislabeled as USD.

Valuation-library research documents now serialize the comparable collection under the canonical top-level `comparables` field, with legacy `comparable_sales` accepted only as a backward-compatible validation alias. Saved-file reruns in the Asset Valuation Detail view load committed `factors.json` and `research.json` from the canonical valuation-library asset directory, show file paths, hashes, comparable counts, repository-committed/runtime-only status, and current-versus-stale input hashes, then revision-back up and regenerate `valuation.json` without requiring pasted intake. Valuation output now records a pre-filter `research_input_summary` with raw and parsed comparable counts and comparable IDs, and every parsed comparable receives a diagnostic row before eligibility selection. If raw research contains comparables that are lost during parsing, the run is classified as `valuation_error` rather than as insufficient evidence.
