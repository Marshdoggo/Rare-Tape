...
- Benchmark Lab provides a reusable, tested analytics boundary for comparing canonical individual-asset observations and exit-aware full-market/category total-return series with public-market proxies. Yahoo Finance chart responses are normalized by an isolated adapter and cached for 24 hours by Streamlit; no downloaded prices are committed. Benchmark observations can be matched exactly, sampled using the previous close known at each Rally evidence date, or reduced to month/quarter ends. Rally observations are never forward-filled. The page exposes growth, relative wealth, rolling excess, period returns, drawdowns, frequency-aware absolute and relative statistics, aligned evidence, and a reproducible CSV export.
...
- Benchmark Lab currently depends on Yahoo Finance's unauthenticated chart endpoint, which offers no application service-level guarantee. It supports canonical asset and built-in total-return index histories; persisted strategy documents need a stable, methodology-owning history resolver before they can safely appear as comparison subjects.
