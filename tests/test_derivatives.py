...
def test_put_call_parity_with_carrying_yield():
    inputs = dict(spot=87, strike=90, time=0.75, rate=0.04, volatility=0.31, yield_rate=0.015)
    call = black_scholes_merton("call", **inputs).price
    put = black_scholes_merton("put", **inputs).price
    expected = inputs["spot"] * __import__("math").exp(-inputs["yield_rate"] * inputs["time"]) - inputs["strike"] * __import__("math").exp(-inputs["rate"] * inputs["time"])
...
def test_zero_time_and_zero_volatility_have_safe_deterministic_values():
    ...
    assert deterministic.price == pytest.approx(100 - 100 * __import__("math").exp(-0.05))
...
def test_irregular_volatility_uses_actual_spacing_without_filling():
    history = pd.DataFrame(
        {
            "date": ["2023-01-01", "2023-01-08", "2023-02-20", "2023-06-30", "2024-01-05"],
            "value": [10, 11, 9, 12, 13],
        }
    )
    ...
    assert estimate.annualized_volatility == pytest.approx(estimate.periodic_volatility * (365.25 / 86.5) ** 0.5)


def test_sparse_volatility_is_unavailable_but_reports_evidence():
    history = pd.DataFrame({"date": ["2024-01-01", "2024-04-01", "2024-07-01"], "value": [10, 11, 12]})
...
def test_dynamic_discovery_deduplicates_dates_and_labels_assets_and_indices():
    assets = pd.DataFrame(
        [{"asset_id": "rally-marx", "ticker": "MARX", "asset_name": "Das Kapital", "category": "books"},
         {"asset_id": "sparse", "ticker": "ONE", "asset_name": "Sparse", "category": "art"}]
    )
    observations = pd.DataFrame(
        {
            "asset_id": ["rally-marx"] * 5 + ["sparse"],
            "observed_at": ["2023-01-01", "2023-04-01", "2023-04-01", "2023-07-01", "2023-10-01", "2023-01-01"],
    ...
    underlyings, warnings = discover_underlyings(assets, observations, indices, minimum_prices=4)
    assert not warnings
...
def test_discovery_handles_missing_and_sparse_data():
    ...
    observations = pd.DataFrame([{"asset_id": "a", "observed_at": "2024-01-01", "price_per_share": None}])
...
def test_term_sheet_is_non_executable_and_contains_required_questions():
    underlying = Underlying(
        "index:test", "Test Index", "Synthetic index — Test", "index", "all", "equal",
        pd.DataFrame({"date": pd.to_datetime(["2024-01-01"]), "value": [100]}), "Test source",
    )
    sheet = term_sheet(underlying, "put", 10, 2, 95, 4.25, date(2024, 1, 1), date(2024, 4, 1), 0.3, 0.04)
